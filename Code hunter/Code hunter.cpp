// Code hunter.cpp : Defines the entry point for the console application.
//
...
int main()
{
	...
	for (int i = 0; i < textToAnalyze.length(); ++i)
	{
		if (textToAnalyze[i] == 'a' || textToAnalyze[i] == 'e' || textToAnalyze[i] == 'i' ||
			...
			textToAnalyze[i] == 'U')
		{
			++vowels; //fixed this to add +1 to vowels as opposed to -1 
		}
		else if ((textToAnalyze[i] >= 'a'&& textToAnalyze[i] <= 'z') || (textToAnalyze[i] >= 'A'&& textToAnalyze[i] <= 'Z'))
		{
			++consonants; //this whole line was a comment so i removed the //
		}
